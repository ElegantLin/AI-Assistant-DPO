...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        logs = dict(
            ...
            loss_ori=next((v for k, v in state.log_history[-1].items() if k.endswith("losses_ori")), None),
            name=next((k for k, v in state.log_history[-1].items() if k.endswith("loss")), None),
            eval_loss=state.log_history[-1].get("eval_loss") or next((v for k, v in state.log_history[-1].items() if k.endswith("loss")), None),
